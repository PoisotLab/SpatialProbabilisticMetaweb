...
df = DataFrame(CSV.File(joinpath("data", "occurrences", "all_occurrences.csv"), delim="\t", types=Dict(:gbifID => String)))
df = DataFrame(CSV.File(joinpath("data", "occurrences", "all_occurrences.csv"), delim="\t", types=Dict(:gbifID => String), quotechar='"'))
...
df = DataFrame(CSV.File(joinpath("data", "occurrences", "all_occurrences.csv"), limit=100, delim="\t", types=Dict(:gbifID => String)))
df = DataFrame(CSV.File(joinpath("data", "occurrences", "all_occurrences.csv"), limit=1_000)) # not reading the right number of rows...
df = DataFrame(CSV.File(joinpath("data", "occurrences", "all_occurrences.csv"), limit=11_800)) # when it stops working

# Attempt with dataset exported with R
df = DataFrame(CSV.File(joinpath("data", "occurrences", "all_occurrences2.csv"), delim="\t"))
# Not 100_000 rows??
# Same with R though...

# Attempt with problem dataset
df = DataFrame(CSV.File(joinpath("data", "occurrences", "all_occurrences_problems.csv"), delim="\t"))
